...
dir_lasso="../" # where the outputs are saved #EDITED LINE 3
...
y_train=keys_merged.loc[i_train[0],'female_pronoun'].to_numpy() #EDITED LINE

### (2) Bring in word count matrix X
word_counts=np.load(dir_data+"X_word_count.npz",encoding='latin1', allow_pickle=True) #EDITED LINE 2
X=word_counts['X'][()] 
X_train=X[i_train[0],:]
X_test0=X[i_test0[0],:]
X_test1=X[i_test1[0],:]
